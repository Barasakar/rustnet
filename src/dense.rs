...
impl Dense {
    ...
    pub fn dense_backward(&mut self, grad_output: &Array2<f32>) -> Array2<f32> {
        ...
        let grad_weights = self.input_cache.as_ref().expect("Bug: potentially running dense_backward before dense_forward").t().dot(grad_output);
    ...
    pub fn dense_update(&mut self, learning_rate: f32) {
        self.weights = &self.weights - learning_rate * self.grad_weights_cache.as_ref().expect("Bug: dense_update is called before dense_backward");
        self.biases = &self.biases - learning_rate * self.grad_biases_cache.as_ref().expect("Bug: dense_update is called before dense_backward");
    }
}
